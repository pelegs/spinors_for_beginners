...
%% -- Find how to properly set up a figure with sub-figures as NON-MARGIN.

The addition rule of vectors is called the parallelogram rule because if we were to perdorm the addition of $\vec{a}+\vec{b}$ and $\vec{b}+\vec{a}$ we would get a parallelogram (\cref{fig:vector_addition_parallogram}). This aslo indicates that vector addition is \textit{commutative}:
\begin{equation}
    \vec{a}+\vec{b} = \vec{b}+\vec{a}.
    \label{eq:vector_addition_commutative}
\end{equation}
...
\begin{marginfigure}
    \begin{tikzpicture}
            ...
            \draw[vector={xblue}] (0,0) -- (b) node[midway, below right] {$\vec{b}$};
            \draw[vector={xblue, dashed}] (a) -- (c) node[midway, above left] {$\vec{b}$};
            \draw[vector={xred, dashed}] (b) -- (c) node[midway, below right] {$\vec{a}$};
            \draw[vector={dashed}] (0,0) -- (c) node[midway, above left, rotate=36.87] {$\vec{a}+\vec{b}$};
    \end{tikzpicture}
    \caption{No matter in which order we perform the addition of two vectors, the result is the same. This is due to the parallelogram rule of vector addition.}
    \label{fig:vector_addition_parallogram}
\end{marginfigure}
