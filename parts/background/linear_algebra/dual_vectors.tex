...
The linearity of the dual vectors space simplifies many calculations - foremost, it allows us to construct dual vectors from other dual vectors using linear combinations. For example, suppose we have two dual vectors
\begin{align}
...
\end{equation}
which is exactly the application of $\dualvec{\alpha}$ on the vector five times followed by the application of $\dualvec{\beta}$ twice.

\subsection{Introducing some formalism}
Now that we know \textit{why} we use dual vectors, we can formalize the concept and find useful properties. Given a vector space $V$ over a field $\mathbb{F}$, its dual space (denoted $\dualspace{V}$) is the set of all the linear functions $\phi:V\to\mathbb{F}$.
